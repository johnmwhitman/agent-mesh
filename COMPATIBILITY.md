...
| Offline A2A delivery-trace profile v0.1 | `fixture-verified` | Pure fixtures normalize modeled stdio, mailbox, HTTP/SSE, and WebSocket labels to one transport-neutral observation trace while preserving canonical identity and explicit non-claims. This implements no live transport, DeliveryPort, authenticated principal, wake authority, persistence, execution, or interoperability. | `docs/A2A-DELIVERY-TRACE-PROFILE-v0.1.md`, `src/a2a/delivery-trace.ts`, `test/fixtures/a2a/delivery-trace/v0.1/corpus.json` |
