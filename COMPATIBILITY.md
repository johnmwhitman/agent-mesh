...
| Offline local-admission evidence-alpha | `reference-conformance` | One test-only raw-text operation binds caller-supplied local evidence and policy fixtures before one injected replay-oracle call. TypeScript and an independent Python witness agree over 70 mandatory cases (44 base + 26 across the Slice 4C-1 exhaustive profile gate closeout); recipient sorting, strict corpus ingestion, mutation canaries, 13 adjacent-pair A00-A13 precedence canaries with 13 later-only controls, every 4A invalid envelope family with exact prefixed source paths, and seven-positive/fourteen-negative static-harness sidecar fixtures are executable. The Slice 4C-1 exhaustive profile gate is CLOSED bounded subfamily across all 11 Section 9 coverage-ledger families. There is no package export, MCP/CLI surface, auth provider, credential proof, replay store, persistence, delivery, runtime, network, public ingress, or remote/multi-host claim. | `docs/A2A-LOCAL-ADMISSION-PROFILE-v0.1.md`, `src/a2a/local-admission.ts`, `reference/python/a2a_local_admission_reference.py`, `test/a2a-local-admission.test.ts` |
...
| Dormant durable acceptance journal | `designed` | The design and migration record are retained, but the acceptance writer is absent from current main. No public ingress, auth provider, delivery, execution, or released durable-acceptance surface is claimed. | `docs/A2A-DURABLE-ACCEPTANCE-v0.1.md`, `docs/adr/0005-dormant-durable-acceptance-journal.md` |
| Slice 4C-0 capability profile and evidence taxonomy | `designed` | The specification and decision record remain, but the former reference implementation is absent from current main. Static harness mapping is tracked separately and does not restore that implementation. | `docs/A2A-CAPABILITY-PROFILE-v0.1.md`, `docs/adr/0006-capability-evidence-is-not-authority.md` |
...
- Banner agreement is observed evidence only. It is not authentication,
  ...
  evidence level.
- Default execution remains OpenCode. Per-agent runtime selection accepts only
  registered adapters and fails closed when a selection is unavailable. There
  is no vendor SDK or catalog, no credential flow, no automatic
  ...
  this public adapter contract and public roadmap. Environment-local model
  observations are operator evidence, not a public availability claim.
