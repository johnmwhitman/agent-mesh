...
export function projectLifecycleOutbox(ownerId: string = randomUUID(), now: number = Date.now()): number {
  ...
  while (true) {
    const row = withLedgerAndStorage((_data, db) => {
      const candidate = db.prepare("SELECT event_id, event, payload FROM lifecycle_event_outbox WHERE projected_at IS NULL AND (claimed_at IS NULL OR claimed_at <= ?) ORDER BY created_at LIMIT 1")
        .get(now - OUTBOX_CLAIM_MS) as { event_id: string; event: string; payload: string } | undefined;
...
export class LifecycleExecutionCoordinator {
  ...
  recover(): void {
    const recovered = withLedgerAndStorage((data, db) => {
      ...
      for (const state of states) this.projectPending(data, state);
      return states;
    });
    void recovered;
    projectLifecycleOutbox(this.ownerId, this.now());
  ...
  private launch(agentId: string): void {
    ...
    void this.runtime.start(spec).then((handle) => {
      ...
      void this.runtime.wait(handle).then((result) => this.settle(agentId, current.attempt_id, current.owner_epoch, result));
  ...
  private settle(agentId: string, attemptId: string, epoch: number, result: RuntimeResult): void {
    ...
    const settled = withLedgerAndStorage((data, db) => {
      ...
      const outcome = success ? store.settle({ workId: agentId, attemptId, ownerId: this.ownerId, ownerEpoch: epoch, outcome: "success", result: result.stdout })
        : store.settleWithRetry({ workId: agentId, attemptId, ownerId: this.ownerId, ownerEpoch: epoch, outcome: "failure", result: result.stdout, error: redact(result.error ?? result.stderr) });
