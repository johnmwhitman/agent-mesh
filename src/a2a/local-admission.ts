...
function normalizeRecipients(recipients: readonly AgentRef[]): AgentRef[] {
  return recipients
    .map((recipient) => ({ namespace: recipient.namespace, agent_id: recipient.agent_id }))
    .sort((left, right) => {
      const namespace = Buffer.compare(Buffer.from(left.namespace, "utf8"), Buffer.from(right.namespace, "utf8"));
      return namespace !== 0
        ? namespace
        : Buffer.compare(Buffer.from(left.agent_id, "utf8"), Buffer.from(right.agent_id, "utf8"));
    });
}
...
export function evaluateLocalAdmission(requestJson: string, envelopeJson: string, replayOracle: ReplayOracle): AdmissionResult {
  ...
  try {
    const decoded = decodeEnvelope(envelopeJson);
    envelope = { ...decoded, recipients: normalizeRecipients(decoded.recipients) };
