...
**Website**: [meshfleet.app](https://meshfleet.app) · **Source version**: 0.20.0 · [npm](https://www.npmjs.com/package/meshfleet) · [CI](https://github.com/johnmwhitman/agent-mesh/actions)
...
`spawn_fleet` also accepts an optional per-agent `runtime`. The default remains `opencode-cli`; non-default runtimes are available only when the operator configures them. `claude-cli` requires an absolute command, a configured version label, and an operator-admitted opaque workspace binding. The caller must repeat that binding as `workspace_binding`, and must omit OpenCode-specific `agent` and `model` selectors. Claude Code owns its OAuth session; Meshfleet never reads or serializes it. The adapter supplies only the operating system's minimal user-profile locator to the scrubbed child so the CLI can find its own login; callers cannot select or override that profile. See [the adapter contract](./docs/ADAPTER-CONTRACT.md#native-claude-code-cli-boundary) for the exact permission and evidence limits. Local smoke tests for any account remain environment-local. The separate pure `recommend_route` advisory may opt in to a caller-evidenced near-reset tie-break, but it never polls an account, grants provider authority, or executes a paid service.
...
[`test/fixtures/corpus/`](test/fixtures/corpus/README.md) is a corpus of 79 deliberately
...
|---|---|---|
| `caught` | 55 | An overclaim — the ledger asserts something its own records don't support. Raises an error and fails the ledger. |
