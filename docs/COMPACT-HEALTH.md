...
With Node 24 and the `meshfleet-0.21.4.tgz` archive in an empty directory:

```sh
npm install --omit=dev --no-audit --no-fund ./meshfleet-0.21.4.tgz
node -p "require('meshfleet/package.json').version"
```

The version should be `0.21.4`. This installs the local archive; it does not
