...
## 21. Acceptance plan — benchmark / migration gates

This section pins the **acceptance gates** for the next slice that promotes
this design into code. The plan is bounded, in-process, and refuses to do
anything the brief forbids: no live bus mutation, no deployment, no credential
rotation, no resident service. Every gate is a contract that a reviewer can
re-run on a fresh checkout.

### 21.1 What "accept" means

A reviewer reads the gate list, runs the four commands named in §21.3, and
each gate either passes or fails deterministically with a printed receipt.
A slice is **accepted** iff every gate reports `passes=true`. Anything
ambiguous (`passes=false`, missing receipt, command exit ≠ 0) blocks the
slice from being merged.

There is no `passes=partial`. A migration that loses one row in 10 000 is a
rejected migration, regardless of how many other gates it passed.

### 21.2 Refusals (the plan does NOT do these)

The acceptance plan explicitly refuses to:

- Touch the live bus store. Every script reads from `$FLEET_BUS_HOME` only
  if that env var is set AND the path equals the live store AND the caller
  passed `--out` to a different path; otherwise it works on a tmp copy.
  This is the same rule as `scripts/fleet-bus-v2-migrate.mjs` §11.
- Promote the live store from v1 to v2. Promotion is operator-gated and
  happens only after John approves the migration receipt (§11.2).
- Start any background process, daemon, systemd unit, launchd plist, or
  cron job. Gates run in the foreground and exit.
- Rotate, create, or read any credential. `~/.hermes/fleet-bus.hostkey` is
  generated by the per-host ceremony (§7.2) only after John signs off; the
  acceptance gates stub the key in-process via `node:crypto.randomBytes`.
- Talk to any external messaging surface, gateway, or remote service.
  Every gate is local-only and works against `:memory:` SQLite or a tmp file.
- Push to `origin/main` or `train/*`. Branch scope is the existing
  `feat/fleet-bus-v2-design-20260821`; the merge-train lands the slice as
  one train commit when Stage B fires.

### 21.3 Gate list (the five gates)

Each gate names the artefact that asserts it, the verifier command, and the
threshold for `passes=true`.

| # | Gate                         | Contract source                  | Verifier                                  | Threshold                                       |
| - | ---------------------------- | -------------------------------- | ----------------------------------------- | ----------------------------------------------- |
| 1 | **Replay**                   | §10.2 + §3                       | contract test #replay*                    | Re-emitted rows preserve `correlation_id`, set `kind='replay'`, and the source row's id is recorded in `causation_id`; one query joins the replay family back to the original by `correlation_id`. |
| 2 | **Ack / retry / DLQ**        | §5.1, §5.3                       | contract test #ack-retry-dlq*             | `ack_ts` advances on success; NACK within `ack_deadline_s` produces a row in `delivery_attempts` with `outcome='nack'` and a monotonic `next_retry_at`; `attempt_no == max_redeliveries` produces a `dead_letter` row with `reason='max_redeliveries'`. |
| 3 | **Causal correlation**       | §6.1–§6.4                        | contract test #correlation-id*            | `bus_thread(correlation_id)` returns rows ordered by `(ts ASC, id ASC)` and the `reply_chain` of the tail equals the concatenation of its ancestors. |
| 4 | **Multi-host writer auth**   | §7.2                             | contract test #writer-key-auth*           | `event_sig = hmac_sha256(hostkey, host_id ‖ … ‖ sha256(subject ‖ body ‖ evidence))`; tampered `body` produces a mismatch and a one-line `dead_letter` row with `reason='policy'`; unsigned v1 rows remain readable with `host_id='legacy:v1'`, `event_sig=null`. |
| 5 | **SQLite → next-store bench**| §14, §16                         | `scripts/fleet-bus-v2-bench.mjs`          | The seven perf targets in the bench script (append ≥ 5 000 r/s, read p95 ≤ 5 ms, migration 10 k rows < 2 s, export 10 k rows < 1 s, replay 1 k < 0.5 s, DLQ insert < 100 ms/event, subscription lag < 200 ms) all pass AND a stub backend implementing §16's `Backend` ABC produces byte-identical `bus_stats` output for the same fixture (the bench script prints `backend_portability: 'match'`). |

Gates 1–4 are wired into `test/fleet-bus-v2-contract.test.ts` and run on
every `npm test` invocation across the Node matrix. Gate 5 is wired into
`scripts/fleet-bus-v2-bench.mjs` and is invoked explicitly:

```
node scripts/fleet-bus-v2-bench.mjs > bench.out
```

The script writes a single JSON receipt to `/tmp/fleet-bus-v2-bench-<ts>.json`
and exits 0 iff every workload passed.

### 21.4 Backend portability (the SQLite → next-store benchmark gate)

Gate 5 is the **only** gate whose threshold involves the future Stage C
backend; this design does not implement that backend, but the bench must
prove the SQLite backend exposes enough surface for a stub backend to
satisfy the same contract.

The `Backend` interface (§16) names eight methods. The bench implements a
`StubBackend` that wraps an in-memory map and a `SqliteBackend` that wraps
the same `:memory:` SQLite used elsewhere in this design. The bench seeds
both with the same 1 000-row fixture (`source`, `source_id`, `topic`,
`correlation_id`, `subject`, `body` — fixed seed), then compares:

- `backend_stats()` JSON shape: identical keys, identical types, byte-equal
  `total` and identical `by_source_kind` distribution.
- `backend_replay(host=None, topic, since_rowid, until_rowid, to_agent)`:
  identical list of `correlation_id`s returned, identical `kind='replay'`
  on the new rows.
- `backend_ack(id, agent)`: identical `delivery_attempts` row appended for
  the same NACK path.

Any divergence prints `backend_portability: 'mismatch'` plus a per-key diff
and the bench exits 1. The SQLite backend passes today; the future NATS
JetStream or Postgres backend only inherits the gate if it ALSO produces
identical output for the same fixture — that is the portability promise.

### 21.5 Receipt format (what every gate prints)

Every gate emits one JSON line on success:

```json
{"gate": "replay", "result": "pass", "ms": 12, "fixture": "T-1", "ts": "..."}
{"gate": "ack_retry_dlq", "result": "pass", "ms": 8, "attempts": 6, "dead_lettered": 2, "ts": "..."}
{"gate": "writer_key_auth", "result": "pass", "ms": 4, "tampered_rows": 1, "ts": "..."}
{"gate": "bench", "result": "pass", "workloads": 7, "ts": "...", "evidence": "/tmp/fleet-bus-v2-bench-<ts>.json"}
```

A failing gate prints `result: "fail"` with the failing assertion in
`reason`. The reviewer is expected to read the receipt first and the code
second; no gate requires a human to chase a stack trace.

### 21.6 What this plan ships

- One new section in this design doc (§21).
- Three new `test(...)` blocks in `test/fleet-bus-v2-contract.test.ts`:
  `#replay*`, `#ack-retry-dlq*`, `#writer-key-auth*`.
- One new workload group in `scripts/fleet-bus-v2-bench.mjs`: the
  `backend_portability` workload plus the seven perf gates already shipped
  on the prior slice.
- No new dependencies. The stub backend is 30 lines of Node.

### 21.7 What this plan does NOT ship

- The Stage B promotion script. Promotion is operator-gated (§11.2); it is
  the next slice, not this one.
- A multi-host bench. The host-key gate (Gate 4) verifies the HMAC against
  a synthetic in-process key, not a real `~/.hermes/fleet-bus.hostkey`.
- A NATS / Postgres backend implementation. Gate 5 proves the SQLite backend
  exposes enough surface for one; it does NOT prove a specific Stage C
  backend is correct.
