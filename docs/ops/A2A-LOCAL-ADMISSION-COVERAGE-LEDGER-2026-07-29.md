...
| envelope | malformed and recipient representatives plus audience/self-recipient ordinary tests | **CLOSED bounded subfamily:** every 4A invalid envelope member class (recipients empty/duplicate/self, sender wildcard, payload non-object/missing media_type, message_id empty, type unknown, audience empty, expires-not-after-issued, scope.fleet_id empty) with exact prefixed source paths incl. `payload.media_type`, `scope.fleet_id`, and `payload.body`; recipient path projection resolves `recipients[index].namespace/agent_id` via the same regex as the Python witness; envelope error projection stays at the family root when no member match is found | — |
