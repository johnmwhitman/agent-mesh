...
Base reviewed: `4a7eb30` (origin/main). Corpus: 52 mandatory raw-text cases in
...
| binding | one invalid field representative | **CLOSED bounded subfamily:** 0 rules (no rule matches the binding context tuple → AUTHORIZATION_DENIED@$); 256 rules (max valid → admission_plan with unchanged 4A digest); 257 rules (cap exceeded → INVALID_BINDING_SNAPSHOT@$.binding_snapshot.rules) | fields, interval edges, duplicate source index, context mismatch |
...
| `binding.rules-256-admit` | `admission_plan` with the unchanged 4A digest (256 rules = max valid; the original matching rule + 255 unique non-matching rules) | 0 |
| `binding.rules-257-reject` | `INVALID_BINDING_SNAPSHOT` at `$.binding_snapshot.rules` (cap exceeded) | 0 |
