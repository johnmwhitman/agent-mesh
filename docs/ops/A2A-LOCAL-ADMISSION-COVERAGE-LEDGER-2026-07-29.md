...
| precedence | representative request, envelope, denial, replay, and expiry ordering | **CLOSED bounded subfamily:** request-wins-envelope; envelope-wins-evidence; denial hides malformed oracle and throw; **mutation canary for every adjacent A00-A13 pair (13 ordinary test mutations, TS + Python witness byte parity)** | every non-adjacent A00-A13 ordering (covered by the corpus's representative precedence cases); A07-A08 and A08-A09 share the public AUTHORIZATION_DENIED collapse so those two pairs cannot watch an order inversion by result bytes |
