...
| binding | one invalid field representative | **CLOSED bounded subfamily:** rule-sender agent-reference grammar (missing namespace, missing agent_id, extra member, non-string agent_id, empty namespace) | fields, interval edges, duplicate source index, context mismatch, and 0/256/257 rule vectors |
| authorization | valid one type/recipient; one invalid action; generic denial | **CLOSED bounded subfamily:** types 5/6; recipients 128/129; duplicate type and recipient source index; all-recipient denial before replay; session + context (adapter / principal / audience / session_ref / sender) mismatch each deny independently; duplicate rule key at rules[1]; duplicate recipient within one rule at recipients[1] | rule-count edge (262144-byte cap with 216-byte rule lower bound makes 2048 rules unrepresentable; existing test pins this) |
...
## Binding rule-sender grammar slice records
...
| `binding.rule-sender-nonstring-agent-id` | `INVALID_BINDING_SNAPSHOT` at `$.binding_snapshot.rules[0].sender.agent_id` | 0 |
| `binding.rule-sender-empty-namespace` | `INVALID_BINDING_SNAPSHOT` at `$.binding_snapshot.rules[0].sender.namespace` | 0 |
