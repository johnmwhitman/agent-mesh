...
| binding | one invalid field representative | **CLOSED bounded subfamily:** snapshot_version (existing `binding.invalid`); snapshot_id; fixture_provenance; effective_from_ms; effective_until_ms — each invalid field representative rejected at its exact path | interval edges, duplicate source index, context mismatch, and 0/256/257 rule vectors |
...
## Binding fields slice records

| Case IDs | Required outcome | Replay calls |
| --- | --- | --- |
| `binding.invalid` | `INVALID_BINDING_SNAPSHOT` at `$.binding_snapshot.snapshot_version` | 0 |
| `binding.field-snapshot-id` | `INVALID_BINDING_SNAPSHOT` at `$.binding_snapshot.snapshot_id` | 0 |
| `binding.field-fixture-provenance` | `INVALID_BINDING_SNAPSHOT` at `$.binding_snapshot.fixture_provenance` | 0 |
| `binding.field-effective-from` | `INVALID_BINDING_SNAPSHOT` at `$.binding_snapshot.effective_from_ms` | 0 |
| `binding.field-effective-until` | `INVALID_BINDING_SNAPSHOT` at `$.binding_snapshot.effective_until_ms` | 0 |
