...
| binding | one invalid field representative | **CLOSED bounded subfamily:** context mismatch (principal_ref differs from authentication evidence) | fields, interval edges, duplicate source index, and 0/256/257 rule vectors |
...
## Binding context-mismatch slice records

| Case IDs | Required outcome | Replay calls |
| --- | --- | --- |
| `binding.context-mismatch` | `AUTHORIZATION_DENIED` at `$` | 0 |
