...
| envelope | malformed and recipient representatives plus audience/self-recipient ordinary tests | **CLOSED bounded subfamily:** every 4A invalid envelope member class — recipients empty/duplicate/self, sender wildcard, payload non-object/missing media_type, message_id empty, type unknown, audience empty, expires-not-after-issued, scope.fleet_id empty (exact prefixed source paths incl. `payload.media_type` and `scope.fleet_id`; envelope error projection stays at the recipient family root — 4A message text carries no recipient index) | — |
