...
- **Local admission Section 9 `request-raw/path` row — cross-object / array-boundary / non-safe-member projection bounded subfamily.** 5 new mandatory corpus cases (corpus 49→54) pinning three properties of the strict raw JSON parser's `RequestScanner` that the previous `request-raw/path` slices did not address: (1) **cross-object duplicate (negative pin)** — `request.cross-object-binding-rules-session-ref` modifies `binding_snapshot.rules[0].session_ref` from `"session-ref"` to `"dup"`; the key `session_ref` now appears in both `authentication_evidence` (value `"session-ref"`) and `binding_snapshot.rules[0]` (value `"dup"`). Each object has its own `keys Set`, so the scanner does NOT detect a cross-object duplicate — the binding tuple mismatch surfaces as `AUTHORIZATION_DENIED` @ `$` with 0 replay-oracle calls. (2) **array-boundary duplicate (negative pin)** — `request.array-boundary-binding-rules-adapter-id` adds a second binding rule that shares the `adapter_id` NAME with `rules[0]` but has a different `principal_ref`; each array element has its own `keys Set`, so the scanner does NOT detect the array-boundary duplicate — `rules[0]` still matches the evidence tuple so the case admits with the unchanged 4A digest and 1 replay-oracle call (envelope_digest matches `valid.admission-plan`). (3) **non-safe-member duplicates (positive pins)** — `request.non-safe-member-auth-evidence-session-ref-dash`, `request.non-safe-member-binding-snapshot-snapshot-id-dash`, `request.non-safe-member-auth-rules-message-types-dash` inject two keys with a non-safe character (`-`) before the canonical key; the scanner's `Set` detects the duplicate but `pathMember()` drops the suffix because the safe-name regex `/^[A-Za-z_][A-Za-z0-9_]*$/` rejects names containing `-`. Projected paths: `$.authentication_evidence`, `$.binding_snapshot`, `$.authorization_snapshot.rules` (the array element path drops `[0]` because the known shape is `[]`, per tick-68 precedent). Each rejects with 0 replay-oracle calls. Family-pin test in `test/a2a-local-admission.test.ts` asserts the 5 ids, the per-case outcome codes and `field_path` projections, the per-case `replay_oracle_calls`, the 4A digest match for the array-boundary admit, the replay-oracle argument parity for the admit, and the ordered id set (red-on-revert guard). Generator `scripts/gen-request-raw-path-cross-object-cases.mjs` (committed) splices after `request.unknown-malformed-child` (semantic-order-preserving); pristine-44 sentinel refuses to splice into a corpus that has already been modified; refuses to double-insert. TS dist 5/5 expected eq actual + Python witness 49/49 ok before splice.
