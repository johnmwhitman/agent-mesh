...
- **Fleet Bus v2 acceptance plan — benchmark/migration gates (§21 of `docs/FLEET-BUS-V2-DESIGN.md`).** Five named gates wire the design doc's promises to deterministic verifiers: (1) replay — re-emitted rows preserve `correlation_id`, set `kind='replay'`, and record source id in `causation_id`; (2) ack/retry/DLQ — `ack_ts` advances on success, NACK within `ack_deadline_s` produces a `delivery_attempts` row with monotonic `next_retry_at`, `max_redeliveries` exhaustion lands in `dead_letter` with `reason='max_redeliveries'`; (3) causal correlation — `bus_thread(correlation_id)` orders by `(ts ASC, id ASC)` and the tail row's `reply_chain` is the closed ancestor list; (4) multi-host writer auth — `event_sig = hmac_sha256(hostkey, host_id ‖ source ‖ source_id ‖ ts ‖ sha256(subject ‖ body ‖ evidence))`, tampered bodies produce a mismatch and a `dead_letter` row with `reason='policy'`, unsigned v1 rows stay readable with `host_id='legacy:v1'` and `event_sig=null`; (5) SQLite-to-next-store bench — seven perf targets in `scripts/fleet-bus-v2-bench.mjs` PLUS a backend-portability workload where an in-memory `StubBackend` implementing §16's surface produces byte-equal `bus_stats` to the SQLite backend. Gates 1–4 are wired into `test/fleet-bus-v2-contract.test.ts` and run on every `npm test` invocation; Gate 5 is wired into `scripts/fleet-bus-v2-bench.mjs` and is invoked explicitly. Plan refuses to touch the live store, deploy, rotate credentials, or start any resident process — same refusal rules as the prior slice. Suite 1791 → 1797 (+6 gate tests).
