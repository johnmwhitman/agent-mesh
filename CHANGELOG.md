...
- **Coverage-ledger depth/numeric subfamily closed.** The local-admission
  evidence-alpha corpus grew from 44 to 51 mandatory raw-text cases: a
  scanner-vs-semantic depth pair at the `MAX_REQUEST_DEPTH = 8` boundary
  ...
  and the independent stdlib-only Python witness agree over the full 51-case
  ...
  `CLOSED bounded subfamily`. No public ingress, auth provider, replay store,
  persistence, MCP/CLI surface, package export, runtime, network, transport,
  delivery, execution, or multi-host behavior is claimed.
