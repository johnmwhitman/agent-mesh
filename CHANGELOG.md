...
- **Section 9 precedence family is closed at the bounded A00-A13 mutation canary level.** The
  local-admission corpus now carries 26 new cases (13 adjacent A00-A13 mutation canaries + 13
  later-only precedence controls) generated by `scripts/gen-precedence-canary-cases.mjs` and
  spliced by `scripts/splice-precedence-canary-cases.mjs`. Each canary mutates the valid
  baseline so the earlier step's failure AND the later step's failure are both present; the
  canary fails closed by being adjacent-ORDER — if a future change reorders the precedence, the
