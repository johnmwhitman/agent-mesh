...
- **Section 9 binding-rule sender-agent-reference grammar + authorization source-indexed duplicate bounded subfamilies.** Seven new mandatory corpus cases pin (a) the binding-rule `sender` agent-reference grammar classes beyond the existing wildcard case — missing `namespace`, missing `agent_id`, extra member, non-string `agent_id`, empty `namespace`, all rejecting at `$.binding_snapshot.rules[0].sender` with `INVALID_BINDING_SNAPSHOT`; and (b) the source-indexed authorization duplicate classes — `authorization.duplicate-key-second-rule` (second-rule key collision at `$.authorization_snapshot.rules[1]`) and `authorization.duplicate-recipient-in-rule` (in-rule recipient duplicate at `$.authorization_snapshot.rules[0].recipients[1]`). TypeScript and the independent Python witness agree byte-for-byte on all 56 cases; the new family-pin test asserts each id, the `INVALID_BINDING_SNAPSHOT` / `INVALID_AUTHORIZATION_SNAPSHOT` code, and the exact source-indexed `field_path`. Coverage ledger binding and authorization rows now read CLOSED for these bounded subfamilies. Generator: `scripts/gen-binding-grammar-cases.mjs` + `scripts/splice-binding-grammar-cases-on-current.mjs` (idempotent, reads `valid.admission-plan`, preserves existing corpus order and compact serialization). Corpus 49 → 56 cases. Suite 1778 → 1779.
