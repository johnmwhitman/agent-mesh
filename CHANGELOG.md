...
- **Tick-111 cherry-pick of the oracle/results "malformed oracle case" closure onto `origin/main` `2c2e392`.** Carries the six `oracle.malformed-*` cases + family-pin test + RED-on-revert guard + Python tuple fix + slice-records table + corpus-count prose bump 49 → 55 across `COMPATIBILITY.md` / `docs/A2A-HANDOFF-CURRENT.md` / `docs/A2A-LOCAL-ADMISSION-PROFILE-v0.1.md` / `docs/A2A-PROGRAM.md` / `docs/ops/A2A-LOCAL-ADMISSION-COVERAGE-LEDGER-2026-07-29.md` from the proven prior-lane union-merge commit `c728e8f` (tick-87's tree) onto the post-merge-train HEAD. The merge-tree is clean against current `origin/main`; the only resolution needed was keeping `origin/main`'s `1778/1778` in `HANDOFF.md` (the post-merge-train figure is larger than the cherry-pick source's `1771/1771`). Post-run guard then bumps `HANDOFF.md` to the measured figure (1778 + 2 oracle-malformed tests = `1780/1780`). No push, merge, deploy, runtime promotion, live-ledger, provider, credential, spend, identity, CarMart, or foreign-worktree action — the cherry-pick sits one commit ahead of `origin/main` on `feat/oracle-malformed-merge-on-coverage-20260819-tick111`, awaiting John's operator-decision merge-train land.
