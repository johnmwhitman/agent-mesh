#!/usr/bin/env node
// Splice the 8 generated relativity cases into the corpus: append cases
// (in id order), append ids to mandatory_case_ids (in the same order), keep
// everything else. Idempotent: refuses to splice if any id already present
// (so a stale /tmp file from a prior run can't half-splice).
