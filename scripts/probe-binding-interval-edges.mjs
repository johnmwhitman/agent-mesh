#!/usr/bin/env node
// Probe the 6 new binding-interval-edges cases against the TypeScript implementation
// and the mandatory Python witness BEFORE splicing into the corpus.
// Usage: node scripts/probe-binding-interval-edges.mjs
...
// Now the witness for the same 6 cases
...
const dir = mkdtempSync(join(tmpdir(), "mf-interval-edges-probe-"));
try {
  ...
  for (const c of cases) {
    ...
    const w = spawnSync("python3", [pythonWitness, "--evaluate-file", path], { encoding: "utf8", timeout: 20_000 });
    if (w.status !== 0) {
      console.log(`PY BAD ${c.id} (witness exit ${w.status}) stderr: ${w.stderr.slice(0,200)}`);
      pyFail++;
      continue;
    }
    const actual = JSON.parse(w.stdout);
    const ok = JSON.stringify(actual) === JSON.stringify(c.expected);
    if (ok) pyPass++; else pyFail++;
    if (!ok) {
      console.log(`PY BAD ${c.id}`);
      console.log("  expected:", JSON.stringify(c.expected));
      console.log("  actual:  ", JSON.stringify(actual));
    } else {
      console.log(`PY OK  ${c.id}`);
    }
  }
