...
// pristine-49 sentinel (origin/main 4a7eb30 baseline). This script is idempotent
// and refuses partial splicing so we never accidentally half-splice a count.
// The generator rejects if the corpus has already been modified past 49 cases.
const pristineSentinel = "/tmp/corpus-pristine-49.json";
const repoRoot = new URL("..", import.meta.url).pathname;
const { execFileSync } = await import("node:child_process");
if (!existsSync(pristineSentinel)) {
  execFileSync("bash", ["-c", `git -C "${repoRoot}" show 4a7eb30:test/fixtures/a2a/local-admission/v0.1/corpus.json > ${pristineSentinel}`]);
}
...
if (corpus.cases.length !== 49) {
  throw new Error(
    `corpus at ${corpus.cases.length} cases (expected pristine 49). ` +
    `Restore from ${pristineSentinel} or rebase onto origin/main.`,
  );
}
