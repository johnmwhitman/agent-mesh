...
def strict_document(source: str) -> Any:
    ...
    if source.startswith("\ufeff"):
        raise ValueError("BOM is not JSON")
    return json.loads(source, object_pairs_hook=reject_duplicate, parse_constant=reject_constant)
...
ENVELOPE_FIELD_RE = re.compile(
    r"\b(sender(?:\.(?:namespace|agent_id))?|recipients(?:\[\d+\])?(?:\.(?:namespace|agent_id))?|payload(?:\.(?:media_type|body))?|scope(?:\.fleet_id)?|extensions|protocol|version|kind|message_id|type|issued_at_ms|expires_at_ms|audience|correlation_id|dedupe_key)\b"
)
...
def evaluate_local_admission(request_json: str, envelope_json: str, replay_oracle) -> dict[str, Any]:
    ...
    if request.get("kind") == "rejected" and set(request) == {"kind", "code", "field_path"}:
        return request
    ...
    try:
        ...
        envelope = {
            **envelope,
            "recipients": sorted(
                ({"namespace": item["namespace"], "agent_id": item["agent_id"]} for item in envelope["recipients"]),
                key=lambda item: (item["namespace"], item["agent_id"]),
            ),
        }
...
def validate_corpus(value: Any) -> list[dict[str, Any]]:
    if not record(value) or set(value) != {"mandatory_case_ids", "cases"}:
        raise ValueError("invalid corpus root")
    mandatory = value["mandatory_case_ids"]
    cases = value["cases"]
    if not isinstance(mandatory, list) or not isinstance(cases, list):
        raise ValueError("invalid corpus arrays")
    ids: list[str] = []
    for case in cases:
        if not record(case) or set(case) != {"id", "api", "invocation_args", "expected"}:
            raise ValueError("invalid corpus case")
        if not isinstance(case["id"], str) or not case["id"] or case["api"] != "evaluate-local-admission":
            raise ValueError("invalid corpus case identity")
        invocation = case["invocation_args"]
        expected = case["expected"]
        if not record(invocation) or set(invocation) != {"request_json", "envelope_json", "replay_oracle_result"}:
            raise ValueError("invalid corpus invocation")
        if not isinstance(invocation["request_json"], str) or not isinstance(invocation["envelope_json"], str):
            raise ValueError("invalid raw corpus input")
        if not record(expected) or set(expected) != {"result", "replay_oracle_calls", "replay_oracle_arguments"}:
            raise ValueError("invalid corpus expectation")
        ids.append(case["id"])
    if any(not isinstance(case_id, str) for case_id in mandatory) or mandatory != ids or len(set(ids)) != len(ids):
        raise ValueError("invalid mandatory case inventory")
    return cases

def run_corpus(path: Path, mutate_output: bool) -> dict[str, Any]:
    corpus = strict_document(path.read_text(encoding="utf-8"))
...
def main() -> int:
    ...
    if args.evaluate_file:
        invocation = strict_document(Path(args.evaluate_file).read_text(encoding="utf-8"))
        if not record(invocation) or set(invocation) != {"request_json", "envelope_json", "replay_oracle_result"}:
