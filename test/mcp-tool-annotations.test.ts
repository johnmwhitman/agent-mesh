...
test("tools/list payload size is measured, default tool count = 36, under 60 KiB", async () => {
  ...
  try {
    ...
    assert.equal(response.tools.length, 36, `expected 36 default tools, got ${response.tools.length}`);
