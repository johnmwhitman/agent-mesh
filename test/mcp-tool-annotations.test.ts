...
test("tools/list payload size is measured, tool count = 38, under 60 KiB", async () => {
  ...
  try {
    ...
    assert.equal(response.tools.length, 38, `expected 38 tools, got ${response.tools.length}`);
