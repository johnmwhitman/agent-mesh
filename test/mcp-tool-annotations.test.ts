...
test("tools/list payload size is measured, tool count = 39, under 60 KiB", async () => {
  ...
  try {
    ...
    assert.equal(response.tools.length, 39, `expected 39 tools, got ${response.tools.length}`);
