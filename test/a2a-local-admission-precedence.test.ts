...
test("Section 9 precedence: mutation canary for every adjacent A00-A13 pair", () => {
  for (const canary of buildAdjacentPairCanaries()) {
    assert.deepEqual(
      observe(canary.request_json, canary.envelope_json, canary.replay_oracle_result),
      canary.expected,
      canary.id,
    );
  }
  for (const control of buildLaterOnlyControls()) {
    assert.deepEqual(
      observe(control.request_json, control.envelope_json, control.replay_oracle_result),
      control.expected,
      control.id,
    );
  }
