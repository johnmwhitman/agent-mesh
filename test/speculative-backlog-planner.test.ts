...
test("exports the speculative backlog planner as a package library subpath", () => {
  assert.equal(
    packageJson.exports["./speculative-backlog-planner"],
...
test("rejects closed-key, bounds, duplicate, and approval-union violations", () => {
  const cases: Array<[string, unknown, RegExp]> = [
    ...
    ["nested route smuggling", input({ tasks: [task("a", { route: { required_capabilities: ["code"], privacy: "network_ok", locality: "any", prompt: "smuggled" } })] }), /tasks\[0\]\.route\.prompt/],
