...
test('whitespace-only ids, fleets and roles are rejected at the write', () => {
  const ledger = freshLedger()
  try {
    assert.throws(() => registerCapability({ agentId: '   ', fleetId: 'f', role: 'r', skills: ['s'] }), /agentId/)
    assert.throws(() => registerCapability({ agentId: 'a', fleetId: '  ', role: 'r', skills: ['s'] }), /fleetId/)
    assert.throws(() => registerCapability({ agentId: 'a', fleetId: 'f', role: '  ', skills: ['s'] }), /role/)
    assert.equal(Object.keys(loadData().capabilities).length, 0)
  } finally {
    ledger.cleanup()
  }
})
