...
//   ~/AI/agents/.hermes/profiles/meshfleet/scripts/fleet_gate.sh
...
test("cron-node-pin: positive control — real Node 24.18.1 proceeds and exports the pin", () => {
  ...
  assert.match(receipt.resolved_command as string, /exec \/Users\/johnwhitman\/AI\/agents\/.hermes\/scripts\/fleet_gate\.sh/);

  // The canonical gate forwards its wakeAgent JSON on the last line.
  const lines = result.stdout.trim().split(/\r?\n/);
  const last = lines[lines.length - 1];
  const parsed = JSON.parse(last);
  assert.equal(parsed.wakeAgent, true, `expected wakeAgent:true, got: ${last}`);
  assert.ok(parsed.context?.fleet_gate === "proceed", `expected context.fleet_gate=proceed, got: ${last}`);
