...
async function waitForTerminalAgent(dir: string, prompt: string): Promise<Record<string, unknown>> {
  ...
  throw new Error(`timed out waiting for Grok agent: ${JSON.stringify(readAgents(dir))}`);
}
...
test("spawn_fleet delivers through an explicitly selected Grok subscription runtime", { skip: POSIX_ONLY }, async () => {
  const dir = mkdtempSync(join(tmpdir(), "meshfleet-grok-spawn-"));
  const command = join(dir, "fake-grk");
  ...
  const server = spawn(process.execPath, [join(repoRoot, "dist/index.js")], {
    ...
    env: {
      ...
      MESHFLEET_GROK_COMMAND: command,
      MESHFLEET_GROK_VERSION: "fixture",
  ...
  try {
    send({
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "grok-runtime-test", version: "1" },
      },
    });
    for (const [id, incompatible] of [
      ...
      send({
        ...
        params: {
          name: "spawn_fleet",
          arguments: {
            agents: [{ role: "reviewer", prompt: "incompatible request", runtime: "grok-cli", ...incompatible }],
          },
        },
      });
    ...
    send({
      ...
      params: {
        name: "spawn_fleet",
        arguments: {
          agents: [{ role: "reviewer", prompt: "return a bounded review", runtime: "grok-cli" }],
        },
      },
    });
    ...
    assert.equal(agent.output, "bounded Grok result");
    assert.deepEqual(agent.runtime_attempts, ["grok-cli"]);
    ...
    assert.equal(
      readFileSync(capturedLabel, "utf8"),
      "Complete the following MeshFleet text task exactly as provided on stdin.",
    );
    ...
    send({
      ...
      params: {
        name: "spawn_fleet",
        arguments: {
          agents: [{ role: "reviewer", prompt: "BLOCKED_WORKER needs input", runtime: "grok-cli" }],
        },
      },
    });
    assert.match(
