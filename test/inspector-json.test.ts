...
test("inspect text events remain byte-compatible", () => {
  ...
  try {
    withCliLedger((dbFile) => {
      // Redirect the child's event log EXPLICITLY rather than by moving its home
      // directory. The old form set only HOME, which is a no-op on Windows
      // (os.homedir() reads USERPROFILE there) — so the child read the machine's
      // REAL event log and this assertion passed only while that file happened to
      // be empty. Naming the file directly removes the platform question instead
      // of answering it twice.
      const result = runInspect(dbFile, ["--events", "2"], {
        HOME: home,
        USERPROFILE: home,
        MESHFLEET_EVENT_LOG_FILE: join(home, "events.log"),
      });
      assert.equal(result.status, 0);
      assert.equal(result.stderr, "");
