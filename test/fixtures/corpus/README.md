...
|---|---|---|
| `caught` | 40 | An overclaim: the ledger asserts something its own records do not support. Must raise its named check at **error** severity and drive `ok: false`. |
...
Together the `caught` and `anomaly` vectors name **all 46** checks the verifier can
