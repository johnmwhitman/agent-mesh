...
function expected(declarations) { const keys = {}; for (const item of declarations) { if (!keys[item.effect_key]) keys[item.effect_key] = {}; keys[item.effect_key][item.effect_digest] = (keys[item.effect_key][item.effect_digest] || 0) + 1; } return { profile: PROFILE, outcome: "classified", groups: Object.keys(keys).sort().map((key) => ({ effect_key: key, classification: Object.keys(keys[key]).length === 1 ? "single_digest" : "digest_conflict", digests: Object.keys(keys[key]).sort().map((value) => ({ effect_digest: value, declaration_count: keys[key][value] })) })) }; }
